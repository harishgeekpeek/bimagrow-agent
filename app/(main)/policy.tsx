                    ...
                    <View className="flex-row items-center p-[5px] gap-2">
                        ...
                        <View className="flex-1 gap-1">
                            <Text ellipsizeMode="tail" className="text-black text-[16px] leading-none font-biorhyme-semibold capitalize">
                                {/* {policyType == 'motor' ? item?.full_name || '' : item?.customer_name || ''} */}
                                {(policyType === "motor"
                                    ? item?.full_name
                                    : item?.customer_name
                                )?.toLowerCase() || "N/A"}
                ...
                {isExpanded && (
                    <View className="px-4 pb-3">
                        <View className="flex-row  items-center gap-2 mb-[5px]">
                            <Text className="text-[14px] text-black font-kanit-medium">{policyType == 'motor' ? item?.vehicle_category_name || '' : item?.lob || ''}</Text>
                            <View className="w-[5px] h-[5px] bg-[#000] rounded-full"></View>
                            <Text className="text-[14px] text-black font-kanit-medium">{policyType == 'motor' ? item?.vehicle_number || '' : item?.plan_name || ''}</Text>
                        </View>
                        <View className="flex-1 mb-[10px]">
                            <Text className="text-[13px] text-[#6C6060] font-kanit-medium">{item?.insurer_name}</Text>
                            <Text className="text-[12px] text-[#6C6060] font-kanit">{policyType == 'motor' ? item?.vehicle_policy_type || '' : 'Policy No.'} - {item?.policy_number || ''}</Text>
                        </View>
                        <Progress value={progress} size="xs" className="bg-[#D4D6DD] mb-[10px]" orientation="horizontal">
                ...
                tabLoading ? (
                    ...
                    <FlatList
                        ...
                        keyExtractor={(item, index) =>
                            item.id ? item.id.toString() : `policy-${index}`
                        }
                                ...
                                {hasMore && (
                                    <View className="items-center">
                                        <TouchableOpacity
                                            ...
                                            className="bg-[#FFBDA4] flex-row gap-2 h-[40px] w-1/3 justify-center items-center rounded-full"
                                        >
                                            {loading && (
